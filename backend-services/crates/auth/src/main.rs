...
use oclink_auth::amqp::AmqpEventPublisher;
use oclink_auth::repository::PostgresTokenRepository;
use oclink_auth::OcLinkAuth;
use oclink_contracts::auth::v1::auth_service_server::AuthServiceServer;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let db_url = std::env::var("AUTH_DATABASE_URL")
        .unwrap_or_else(|_| "postgres://postgres@localhost:5432/oclink_auth".to_string());
    ...
    let service = OcLinkAuth::new(repo, event_publisher);
