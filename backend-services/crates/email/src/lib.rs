...
use oclink_contracts::email::v1::email_service_server::EmailService;
use oclink_contracts::email::v1::{
...
#[tonic::async_trait]
impl EmailService for OcLinkEmail {
    ...
    async fn set_email(
        ...
        if let Some(r) = record {
            if r.is_verified {
                ...
                // For verified emails, execute a multi-step change by first challenging the old email address.
        ...
        // Overwrite or create state directly if the record does not exist or remains unverified.
    ...
    async fn verify_email(
        ...
        if expired || record.verification_code.as_deref() != Some(inner.code.as_str()) {
            return Ok(Response::new(VerifyEmailResponse { success: false }));
        }
        ...
        match record.verification_type.as_deref() {
            ...
            Some("CONFIRM_OLD") => {
                // Transition the state machine from validating the old email address to verifying the new one.
                let code = Self::generate_code();
                let expires = OffsetDateTime::now_utc() + Duration::minutes(15);
                self.repo
