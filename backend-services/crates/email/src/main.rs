//! Binary entry point for the OcLink email identity microservice.
//!
...
use oclink_contracts::email::v1::email_service_server::EmailServiceServer;
use oclink_email::amqp::AmqpBroker;
use oclink_email::repository::PostgresEmailRepository;
use oclink_email::worker::start_email_worker;
use oclink_email::OcLinkEmail;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let db_url = std::env::var("EMAIL_DATABASE_URL")
        .unwrap_or_else(|_| "postgres://postgres@localhost:5432/oclink_email".to_string());
    let amqp_url =
    ...
    let service = OcLinkEmail::new(repo, broker);
